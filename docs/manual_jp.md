...
この数値はオーダーパラメーター (秩序変数, [order parameters](https://en.wikipedia.org/wiki/Phase_transition#Order_parameters)  ) とも呼ばれます。
似通った並び方の粒子は同じ数値になり、違った並び方をしている粒子は違う数値になります。
体心立方格子 (body-centered cubic, BCC) や面心立法格子 (face-centered cubic, FCC) ではオーダーパラメーターが特定の値になることが知られており、
...
## 必要なデータ
  ...
  1. coord                     : 各粒子の座標のリスト。  [ [0,0,0], [0,0,1], ... ]  
...
## すべてのオーダーパラメーターで共通の設定

### 隣接粒子の設定

各粒子のオーダーパラメーターの数値は近くに存在する粒子の座標を使用して計算されます。
このある粒子 $i$ の近くにある粒子のことを、ある粒子の隣接粒子と呼びます。
隣接粒子の決める方法として、このスクリプトでは3つの方法が実装されています。  

1つめの方法は、粒子$i$からの距離がある距離 (隣接半径) 以内にある粒子を、粒子$i$の隣接粒子とする方法です。
この方法の利点は、実装が比較的簡単でユーザーにとってもわかりやすいことです。
欠点としては、隣接粒子の個数が各粒子で違う場合がある事と、スケール依存性があることです。
例えば、隣接半径を1 nm と1 angstrom を指定するのでは結果が変わります。これをスケール依存性があると言います。  
2つ目の方法は、粒子$i$から距離の近い$n$個の粒子を隣接粒子とする方法です。
利点としては、スケール依存性がなく、長さの単位が変わっても距離の大小関係が変わらないため、隣接粒子が変化しません。
欠点としては実装が簡単ではない、計算コストが数割程度大きい、論文での使用実績が存在するが少ないという点です。  
3つ目の方法はDelaunay分割 (Voronoi分割, [Voronoi diagram](https://en.wikipedia.org/wiki/Voronoi_diagram))を使う方法です。
Voronoi分割を行った後、各Voronoi面を共有する2つの粒子を隣接粒子とします。

各方法の使用法は、以下の通りです。

#### 隣接半径を指定する場合

オーダーパラメーターの計算条件を指定するop_settingsで次のように指定を行います。
...
上の例では、隣接半径=1.5, 1.75での指定を行っています。  

#### 隣接粒子の数を指定する場合

オーダーパラメーターの計算条件を指定する op_settings で次のように指定を行います。
...
粒子$i$のオーダーパラメーターとその隣接粒子のオーダーパラメーターで平均を計算する場合があります。
粒子$i$の$N$個の隣接粒子のlistを$N_b(i)$、 $N_b(i)$に粒子$i$自身を追加した$N+1$個の粒子のlistを $\tilde{N}_b(i)$とすると、
ーダーパラメーターを$X(i)$,平均化した後のオーダーパラメーターを$Y(i)$とすると、次の様な計算を行います。
$$Y(i) = \frac{ \sum_{j \in \tilde{N}_b(i)}X(j)}{N+1}$$

この平均化の操作には以下の利点と欠点があります。
利点としては、平均を計算するだけであるため計算負荷が軽い事、同じオーダーパラメーターの分類性能を上げる事が可能な事、
何らかの数値であれば使用できる事、何回も平均化をする事が可能である事などです。
一方、欠点としては、オーダーパラメーターの計算に使用する情報が多くなり、オーダーパラメーターの解像度が下がる事です。
オーダーパラメーターは隣接粒子の情報を使って計算され、さらに平均化の計算では隣接粒子のオーダーパラメーターを使って計算されます。
平均化操作を1回行うごとに、隣接粒子の隣接粒子の情報を使用することになる。
つまり、オーダーパラメーターの計算に使用する粒子が多くなり、遠くの粒子の情報を使用することになる。
...
この平均化を行います回数は、オーダーパラメーターの計算条件を指定する op_settings で次のように指定を行います。
...
変数$type$で計算を行うオーダーパラメーターの種類を指定します。

隣接粒子数12、隣接半径2.0でのオーダーパラメーターの計算条件として以下の例を示します。
...
隣接粒子数12、隣接半径2.0でのオーダーパラメーターの計算条件として以下の例を示す。
...
隣接粒子数4、隣接半径2.0でのオーダーパラメーターの計算条件として以下の例を示す。
...
変数$a$は、オーダーパラメーターの値を隣接粒子で平均化を行う回数です。
変数$N_b(i)$は粒子$i$の$N$個の隣接粒子のリストです。
...
Steinhardtのオーダーパラメーター[@Steinhardt1983]は以下のオプションで計算できる。
...
Lechnerのオーダーパラメーター[@Lechner2008]は以下のオプションで計算できる。
...
また、オーダーパラメーター$W2$に関しては以下のように、計算する。
...
隣接条件 Delaunay分割を利用した場合、Mickelのオーダーパラメーターを利用することができる。以下にオプションを示す。 [@Mickel2013]
...
このオーダーパラメーターは、[@Moore2010][@Fitzner2020][@Tribello2017]を模して実装したものです。
...
隣接粒子数12、隣接半径2.0でのオーダーパラメーターの計算条件として以下の例を示す。
[@Moore2010]と同じようなオーダーパラメーターは以下のオプションで計算できる。
...
Onsagerのオーダーパラメーター $S$ は次の式で計算される。[@Onsager1949][@Zannoni1979]
...
$n = 2, 4$の時、オーダーパラメーター $S$ はそれぞれ以下の式で計算される。
$$ S^{(a=0, n=2)}(i) = \frac{ \sum_{j \in N_b(i)} { [ 3 \cos^2(\theta) - 1]/2 } }{N} $$
$$ S^{(a=0, n=4)}(i) = \frac{ \sum_{j \in N_b(i)} { [ 35 \cos^4(\theta) -30 \cos^2(\theta) + 3  ]/8 } }{N} $$

隣接粒子数12、隣接半径2.0でのオーダーパラメーターの設定として以下の例を示す。
尚、このオーダーパラメーターは計算のために方向ベクトルが必須であるため、質点や球状粒子の解析に使う事はできない。
...
McMillanのパラメーター$T$は次の式で計算される。[@McMillan1971]
...
$n = 2$の時、オーダーパラメーター $T$ は以下の式で計算される。
...
ユーザーが定義するためのオーダーパラメーター $Z$ を用意している。
...
オーダーパラメーターの計算部分としては、 op_tools/op_z_user_define.py です。
このファイルを編集し、ユーザーの考えたオーダーパラメーターを実装することが可能です。
...
```python
  ...
  op_settings = {  
    ...
    'neighbor'       : [8],        # 隣接粒子数
    'radius'         : [1.5],      # 隣接半径
    'ave_times'      : 1,          # あるオーダーパラメーターを周囲の粒子で平均を計算する回数  
    # A
    'op_types' : ['A','P','N'],    # オーダーパラメーター A での解析の種類の指定
    ...
    # C
    'types_in_C' : ['half'],       # ゆらぎを表現したベクトルを何回足すかというパラメーター
    ...
    'l_in_Q'         : [2, 4, 6],  # 球面調和関数の次数を指定するパラメーターl  
    # Q2 W2
    ...
    'Q', 'W', 'Q2', 'W2', 'LQ', 'LW', 'S', 'T']} # 解析するオーダーパラメーターの種類
```
...
出力として各粒子のオーダーパラメーターが計算される。
各粒子のオーダーパラメーターには次のようにアクセスできる。
...
- Q : オーダーパラメーターの種類
- N6 : 隣接粒子の条件
- 'a=1_b=1' : オーダーパラメーターの計算に使用したパラメーター一覧
